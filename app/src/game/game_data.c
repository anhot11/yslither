...
static const bool CG_COLORS_CT[NUM_COLOR_GROUPS] = {
...
static const vec3s CG_COLORS[NUM_COLOR_GROUPS] = {
    [0] = {0.75f, 0.5f, 0.99609375f},
...
static const uint8_t DEFAULT_SKINS[NUM_DEFAULT_SKINS][64] = {
    [0] = {1, 0},
    [1] = {1, 1},
    [2] = {1, 2},
    [3] = {1, 3},
...
static const char NTL_CG_MAP[NUM_COLOR_GROUPS] = {
    [0] = 'z',  [1] = 'x',  [2] = 'c',  [3] = 'v',  [4] = 'b',
    [5] = 'n',  [6] = 'm',  [7] = ',',  [8] = 'a',  [9] = 's',
...
void game_data_init(tenv* env) {
  ...
  memcpy(gdata->cg_colors_ct, CG_COLORS_CT, sizeof(gdata->cg_colors_ct));
  memcpy(gdata->cg_colors, CG_COLORS, sizeof(gdata->cg_colors));
  memcpy(gdata->default_skins, DEFAULT_SKINS, sizeof(gdata->default_skins));
  memcpy(gdata->ntl_cg_map, NTL_CG_MAP, sizeof(gdata->ntl_cg_map));

  static const int U_M[7] = {64, 32, 16, 8, 4, 2, 1};
  memcpy(gdata->u_m, U_M, sizeof(gdata->u_m));

  gdata->SHADOW_UV = (vec4s){0, 6 / 9.0f, 1.5f / 7, 1.5f / 9};
  gdata->CURSOR_UV = (vec4s){2 / 7.0, 7 / 9.0, 1 / 7.0, 1 / 9.0};
  gdata->SKIN_LESS_UV = (vec4s){6 / 7.0, 6 / 9.0, 1 / 7.0, 1 / 9.0};
